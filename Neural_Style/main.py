...
class Config(object):
    alpha = 0.1 # default alpha for network

    image_size = 256  # img_size
    batch_size = 8
    # image_size = 128
    # batch_size = 16
    data_root = 'data/'  # data set path：data/coco/a.jpg
    num_workers = 4  # 多线程加载数据
    use_gpu = True  # 使用GPU

    flag = 1
    style_path = 'styles/style1.jpg'  # 风格图片存放路径
    lr = 1e-3  # 学习率
...
def train(**kwargs):
    ...
    transfroms = tv.transforms.Compose([
        ...
        tv.transforms.ToTensor(), #totensor会把图片从(0,255)转换到(0,1)
    ...
    # 转换网络
    transformer = TransformerNet(opt.alpha)
    if opt.model_path:
    ...
    for epoch in range(opt.epoches):
        ...
        for ii, (x, _) in tqdm.tqdm(enumerate(dataloader), total=len(dataloader)): #tqdm用于加载进度条
        ...
        #if (ii + 1) % opt.plot_every == 0:
        vis.save([opt.env])
        t.save(transformer.state_dict(), 'checkpoints/%s/%s_style.pth' % (opt.flag, epoch))
